...
function Gpio() {
     ...
     */
    function _write(channel, value, cb) {
        ...
        var targetPath = PATH + '/gpio' + pin + '/value';
        if (cb) {
            fs.writeFile(targetPath, value, cb)
        } else {
    ...
    this.write = this.output = function(channel, value, cb /*err*/) {
        ...
        _write(channel, value, cb);
    };

    this.writeSync = function(channel, value) {
        _write(channel, value);
    };

    function _read(channel, cb) {
        var pin = getPinForCurrentMode(channel);
        ...
        if (cb) {
            fs.readFile(targetPath, 'utf-8', function(err, data) {
                if (err) {
                    return cb(err)
                }
                data = (data + '').trim() || '0';
            ...
            data = fs.readFileSync(targetPath, 'utf-8');
            data = (data + '').trim() || '0';
            debug('read pin %s with value %s', pin, data);
            return data === '1';
        }
    ...
    this.readSync = function(channel) {
